    ...
    if (!$result_select) {
        ...
        if (empty($columns)) {
            ...
            if ($existing_record) {
                ...
                $sql_update = "UPDATE $unit SET " . implode(", ", $set_clause) . " WHERE tahun = '$tahun'";
                $result_update = mysqli_query($conn, $sql_update);

                if ($result_update === false) {
                    echo "<script>alert('Error updating existing record: " . mysqli_error($conn) . "');</script>";
                } else {
                    echo "<script>alert('Existing record updated successfully for date: " . $tahun . "');
                ...
                $sql_insert = "INSERT INTO $unit (tahun, $columns_sql) VALUES ('$tahun', $values_sql)";
                $result_insert = mysqli_query($conn, $sql_insert);

                if ($result_insert === false) {
                    echo "<script>alert('Error inserting new record: " . mysqli_error($conn) . "');</script>";
                } else {
                    echo "<script>alert('New record submitted successfully for date: " . $tahun . "');
                            window.location.href = window.location.href;
                            </script>";
                }
            }
